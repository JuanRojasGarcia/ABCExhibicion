...
set @iMessage = 0
	 ...
	 ELSE IF @iOpcion = 2
	 BEGIN
		UPDATE zabccat_ventas SET idu_empleado = @iClienteid, num_totalventa = @iTotalVenta, fech_venta = @iFechaVenta  where idu_venta = @iVentaid
	 END
	 ELSE IF @iOpcion = 3
	 BEGIN
	 ...
	 ELSE IF @iOpcion = 6
	 BEGIN
		select idu_empleado, nom_empleado, nom_apellido from zabccat_empleados 
	 END
	 ELSE IF @iOpcion = 7
	 BEGIN
	 ...
	 ELSE IF @iOpcion = 8
	 BEGIN
		select rfc_empleado from zabccat_empleados where idu_empleado = @iClienteid 
	 END
	 ELSE IF @iOpcion = 9
	 BEGIN
